...
- **Round.**
  - `number: 1..8`
  - `phase: 'commit' | 'standoff' | 'withdraw' | 'reveal_bbb' | 'reveal_others' | 'split'`
...
The state machine is **phased**, not free-form — the round resolver progresses through phases in fixed order, and each phase has clear entry/exit conditions. This lets later variants insert phases (e.g. the cop variant's "telephone" phase) without restructuring.
...
## Round flow (the 6 phases)

Each round runs the same loop. The commit phase is the only untimed one — it's the disconnect-pause boundary, and it's where the loot for this round is revealed. After split finishes, the next round's commit phase begins automatically (or the end-game screen appears if game-end conditions are met). There is **no separate recap/ready beat** between rounds.

### Phase 1 — Load & aim / commit (untimed)
On entry, the big screen draws 5 banknotes from the deck and lays them face up alongside any unsplit carryover from the previous round (~1.5s reveal animation). Standing-but-laid-down players from the previous round pop back upright.
...
When all alive players have committed, phase 2 starts automatically.

### Phase 2 — Standoff countdown (theatrical, 3-2-1)
Pure theatrics. Targets are *already locked* from phase 1 — this countdown does not accept input and has no commitment deadline.
...
### Phase 3 — Cojones / withdraw (3-2-1 countdown)
...
### Phase 4 — Reveal bang!bang!bang! (auto, dramatic)
...
### Phase 5 — Reveal bang & clic (auto, dramatic)
Big screen reveals the remaining cards, simultaneously, paced after the B!B!B! moment. `bang` hits land — target takes a wound, lays down. `clic` does nothing (small comic beat / sigh of relief). Multiple bangs on the same target stack into multiple wounds. A player wounded in phase 4 can still take more wounds in phase 5.

### Phase 6 — Split the loot (auto)
...
After the split animation finishes, the next round's commit phase begins automatically — no per-round ready/recap pause. (If game-end conditions are met instead, the big screen advances to the end-game state.)
...
- **Targeting lines** (drawn at "0" of phase 2 standoff countdown, persist through phases 4 and 5) — animated arrows from shooter to target.
...
- **Phase 1 (load & aim):** two pickers — remaining bullets (tap one) and alive opponents (tap one). Both must be selected, then a Ready tap locks them in. After commit, show "weapon loaded, target acquired — waiting for others". Selections may be changed freely until Ready is tapped. The picker doubles as the per-player "what's left in your hand" reminder, so no separate recap is needed.
- **Phase 2 (standoff countdown):** crosshair / gun-barrel aesthetic confirming the locked target ("Aiming at: [name]"). No input. Live countdown visible.
- **Phase 3 (withdraw):** big DUCK button + indicator of who is aiming you. Live countdown. Decision accepted until 0.
- **Phase 4/5 (reveal):** read-only view of the big screen action; haptic vibration on getting shot or hitting someone.
...
- **Disconnect mid-round.** Don't pause mid-countdown — finish the current real-time phase. The natural pause boundary is the untimed commit phase (phase 1) — if a player is disconnected and hasn't committed there, the phase blocks until they reconnect. Big screen shows "Waiting for [Player] to reconnect…". Players are co-located, so this is a real-world coordination problem — they'll know whose phone died. There is no "bullet lost / too slow" penalty in v1: untimed commits remove the time pressure that the rulebook used to enforce that rule.
